...
function data = analyzeExperimentData_noControlShiftLS(csvFilePaths, ...
    dependentVars, deleteVars, deleteRows)
    ...
    data = loadAndPreprocessData(csvFilePaths);
    
    if ~isempty(deleteVars)
        disp('Delete columns from table')
        data = removevars(data, deleteVars);
    end
    if ~isempty(deleteRows)
        disp(['Delete rows: tbl.', deleteRows{1}, ' == ', deleteRows{2}]);
        data(data.(deleteRows{1}) == deleteRows{2}, :) = [];
    end
    disp('Remove CCIdiff baseline')
    try
        data(data.CCIdiff == 0) = [];
    catch
        disp('Could not remove CCIdiff equals to 0')
    end
...
function runMixedModels(data, dependentVar)
    ...
    for i = 1:length(availableFactors)
        ...
        try
            ...
            resanova = anova(lme, 'DFMethod', 'Satterthwaite');
            disp(resanova)
...
function visualizeResults(data, dependentVar)
    ...
    colorFactors{2} = [.2 .4 .6;.3 .5 .8;.2 .4 .6;.3 .5 .8]; % direction
