...
folder = 'Data/LocalSurround/Indoor/Khaki/';
subdir = dir(folder);
...
save_file_name = 'khaki_indoor.mat';

num_sessions =   [1 1 2]; % 1.*ones(8,1);%[1 1 1 1 1 2 1 1 1 1 1 1 1];% 
...
for i = 1:length(CSVtable)
    ...
    Competitors{j}, saveFig);
        ...
        axis('square')
        axis([0 1 0 1]);
        title(['Ill:', num2str(j),', R^2=', num2str(Rsq1)]);
...
%% Save in a mat file
save(save_file_name, 'new_lab_color', 'CCI','targetCompetitorFit', ...
    'logLikelyFit','predictedResponses', 'Matrix');
