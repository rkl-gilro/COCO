...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        elseif confs.fill==1
            ...
            [~,~,ch] = size(confs.colors);
            if ch > 1
                colors = confs.colors(g, g, :);
            else
                colors = confs.colors(g,:);
            end
            h.bx(k,g) = fill([Xx(:,1)' Xx(1,:) Xx(:,2)' Xx(2,[2,1])],...
                 [Yy(:,1)' Yy(1,:) Yy(:,2)' Yy(2,:)],colors);            
    ...
    if confs.linkline==1
        ...
        h.ln(g) = line(gpos(g,:),pt(4,:),'color',colors,...
    ...
    %% Keep median for each group
    median_g(g) = box_mdcor(1);
end
% move lines to the background
if confs.linkline==1
    uistack(h.ln,'bottom')
    h.ln = line(gpos,median_g,'color',[.6 .6 .6],...
            'LineStyle','-.','LineWidth',3.5);
end
