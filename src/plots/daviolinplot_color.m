...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        if strcmp(confs.violin,'full')            
            h.ds(k,g) = fill([f,-fliplr(f)]+gpos(g,k),...
                [xi,fliplr(xi)],confs.colors(g,k,:));            
        elseif strcmp(confs.violin,'half')   
            if confs.box==3
                h.ds(k,g) = fill(1.3.*f+gpos(g,k),xi,confs.colors(g,k,:));
            else
                h.ds(k,g) = fill(f+gpos(g,k),xi,confs.colors(g,k,:));
            end 
        end       
        ...
        if confs.box~=0
            ...
            if strcmp(confs.boxcolors,'same')
                h.bx(k,g) = fill([Xx(:,1)' Xx(1,:) Xx(:,2)' Xx(2,[2,1])],...
                     [Yy(:,1)' Yy(1,:) Yy(:,2)' Yy(2,:)],confs.colors(g,k,:)); 
            else
                h.bx(k,g) = fill([Xx(:,1)' Xx(1,:) Xx(:,2)' Xx(2,[2,1])],...
        ...
        if confs.scatter~=0 
            if strcmp(confs.scattercolors,'same')
                h.sc(k,g) = scatter(xdata(~ox),data_vals(~ox),...
                    confs.scattersize,...
                    'MarkerFaceColor', confs.colors(g,k,:),...
                    'MarkerEdgeColor', 'k',...
    ...
    if confs.linkline==1
       h.ln(g) = line(gpos(g,:),pt(4,:),'color',confs.colors(g,k,:),...
