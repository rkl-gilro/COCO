...
data1{1} = randn([30,2]);   % Humans
data1{2} = randn([40,2]);   % Dogs
data1{3} = randn([50,2]);   % God
...
group_inx3{1} = [ones(1,30), 2.*ones(1,30) 3.*ones(1,30)];
group_inx3{2} = [ones(1,20), 2.*ones(1,20) 3.*ones(1,20)];
% new data in a cell array with group differences 
data3{1} = randn([30,3]) + (0:0.5:1);    % Humans
data3{2} = randn([20,3]) - (2:2:6);      % Dogs
data3{3} = randn([10,3]) - (2:2:6);      % Dogs
...
subplot(3,2,6)
h = daviolinplot(data3,'groups',group_inx3,'colors',c,'box',3,...
