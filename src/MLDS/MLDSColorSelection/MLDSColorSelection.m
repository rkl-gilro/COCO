...
% the A matrix above.
% disp 'Condition Ax < b changed'
b = -sigma/sigmaFactor*ones(numberOfCompetitors-2,1); % -ones(numberOfCompetitors-2,1); % 
...
options = optimset('Diagnostics','off','Display','off','LargeScale','off','Algorithm','active-set'); %
...
% disp 'Change upper bound and initialCompetitorPositions!!!'
...
for k1 = 1:length(trySpacings)
    ...
    for k = 1:length(tryTargetPositions(k1,:))
        ...
        vub = 100*max(abs(initialParams))*ones(size(initialParams)); % 100*  %% RG added the one, before 100*
...
% disp(['Best k: ', num2str(k), ' and k1: ', num2str(k1)])
end
