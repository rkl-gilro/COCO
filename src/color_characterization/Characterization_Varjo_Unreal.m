...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('Calib_UnrealStandard521_Varjo_13_06_2024.mat');
save_filename = 'Calib_UnrealStandard521_Varjo_14_06_2024_LUT_dE.mat';

%% Comment or uncomment accordingly
x = (0:5:255)./255;

primaries(1, :) = [Red];
% primaries(1, 3) = [Red(2)]; % Calib_UnrealStandard521_Varjo_10_06_2024_2
primaries(2, :) = [Green];
primaries(3, :) = [Blue];
primaries(4, :) = [Gray];

% primaries(1, :) = [Red Red(end)];
% primaries(2, :) = [Green Green(end)];
% primaries(3, :) = [Blue Blue(end)];
% primaries(4, :) = [Gray Gray(end)];
...
for i=1:size(primaries, 1)
    ...
    % plot(xs(:, i), ys(:, i), 'o', 'MarkerSize', 12, ...
    %     'MarkerEdgeColor', 'k', 'MarkerFaceColor', ...
    %     cols{i}.*repmat(x, 3, 1)', 'LineWidth', .3);
    scatter(xs(:, i), ys(:, i), 150, (cols{i}.*repmat(x, 3, 1)').^.45,...
        'filled','LineWidth',.1);
...
for i=1:size(primaries, 1)
    ...
    % for j=1:size(primaries, 2)
        ...
        % plot(x,Ys(:, i), 'Color', cols{i}, 'o'); hold on
        scatter(x, Ys(:, i), 150, (cols{i}.*repmat(x, 3, 1)').^.45,...
        'filled','LineWidth',.1);
...
disp(num2str([mean(dE(dE<4)) median(dE(dE<4)) std(dE(dE<4)) min(dE(dE<4)) max(dE(dE<4))]))
